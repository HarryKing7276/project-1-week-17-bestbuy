...
public class ProductsExtractionTest {
    ...
    @Test
    public void test034() {
        List<HashMap<String, ?>> totalNumber = response.extract().path("data.find{it.name = 'Duracell - AA 1.5V CopperTop Batteries (4-Pack)'}.categories");
    ...
    @Test
    public void test036() {
        List<HashMap<String, ?>> nameOfProduct = response.extract().path("data.findAll{it.name = 'Energizer - MAX Batteries AA (4-Pack)'}.categories.name");
    ...
    @Test
    public void test39() {
        List<Integer> price = response.extract().path("data.findAll{it.price>5.99}.categories.createdAt");
        System.out.println(price);
    }
